...
//只用'a,'b,'c 个不同的生命周期才行
pub fn parse_with_regex<'a:'b, 'b, 'c>(s: &'b str, keyword: &'a Regex, regex: &'a Regex, ret: &'a mut Vec<&'c str>) -> Value {
...
fn process<'a:'b, 'b, 'c>(toker: &'b mut Tokenizer, is_key_matched: bool, keyword: &'a Regex, regex: &'a Regex, ret: &'a mut Vec<&'c str>) -> Value {
    if toker.ch.is_none() {
        return Value::default();
    }
    toker.skip_bc();
...
fn proc_bool_null<'a:'b, 'b, 'c>(toker: &'b mut Tokenizer, is_key_matched: bool, keyword: &'a Regex, regex: &'a Regex, ret: &'a mut Vec<&'c str>) -> Value {
    let step_n;
...
fn proc_number<'a:'b, 'b, 'c>(toker: &'b mut Tokenizer, is_key_matched: bool, keyword: &'a Regex, regex: &'a Regex, ret: &'a mut Vec<&'c str>) -> Value {
    toker.skip_bc();
    if toker.ch.as_ref().unwrap().ch == '-' {
        toker.concat();
        toker.get_char();
...
fn proc_array<'a:'b, 'b, 'c>(toker: &'b mut Tokenizer, is_key_matched: bool, keyword: &'a Regex, regex: &'a Regex, ret: &'a mut Vec<&'c str>) -> Value {
    toker.get_char();
    toker.skip_bc();
    if toker.ch.as_ref().unwrap().ch == ']' {
        toker.get_char();
...
fn proc_string<'a:'b, 'b, 'c>(toker: &'b mut Tokenizer, is_key_matched: bool, keyword: &'a Regex, regex: &'a Regex, ret: &'a mut Vec<&'c str>) -> Value {
    toker.get_char();
    if toker.ch.as_ref().unwrap().ch == '"' {
...
fn proc_object<'a:'b, 'b, 'c>(toker: &'b mut Tokenizer, is_key_matched: bool, keyword: &'a Regex, regex: &'a Regex, ret: &'a mut Vec<&'c str>) -> Value {
    toker.get_char();
    toker.skip_bc();
    if toker.ch.as_ref().unwrap().ch == '}' {
        toker.get_char();
